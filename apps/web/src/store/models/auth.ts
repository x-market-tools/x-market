...
export interface AuthModel {
    ...
    disconnect:Thunk<AuthModel,{link:ProtonWebLink | Link | null | undefined,session:LinkSession | null | undefined}>;
...
export function configureAuthModel(config: AuthModelConfiguration): AuthModel {
    ...
    const authModel: AuthModel = {
        ...
        disconnect: thunk(async (actions, account) => {
            ...
            account.link.removeSession(process.env.NEXT_PUBLIC_PROTON_REQUEST_ACCOUNT!, account.session.auth, account.session?.chainId);
        ...
        fetchProtonProfile: thunk(async (actions,actorName) => {
            ...
            if (protonAvatar) {
                ...
                actions.setProtonProfile({avatar:protonAvatar.avatar,userName:protonAvatar.name})
                actions
            }
        }),
        //TODO: Mark as async when implement
